...
const env = {
  ...
  PANAPHEUS_STATE_TOKEN: 'state-secret',
  PANAPHEUS_DB: fakeD1(store)
};
const calls = [];
...
const challenge = await worker.fetch(new Request('https://panapheus.test/', {
  method: 'POST',
...
const eventBody = JSON.stringify({
  ...
  event: {
    ...
    user: 'U123',
    ts: '123.456',
    text: '*The* _quick_ ~brown~ fox jumps over the lazy dog https://example.com <https://example.com|label>'
  }
});
...
async function signedRequest(body, contentType) {
  ...
  return new Request('https://panapheus.test/slack', {
    method: 'POST',
    headers: {
  ...
  return {
    prepare(sql) {
      return {
        ...
        async run() {
          if (sql.startsWith('INSERT INTO panapheus_state')) {
            const [key, value, expiresAt] = this.params;
            values.set(key, { value, expires_at: expiresAt ?? null });
          } else if (sql.startsWith('DELETE FROM panapheus_state WHERE key = ?')) {
            values.delete(this.params[0]);
          } else if (sql.startsWith('DELETE FROM panapheus_state WHERE expires_at')) {
        ...
        async first() {
          if (!sql.startsWith('SELECT value, expires_at FROM panapheus_state')) return null;
